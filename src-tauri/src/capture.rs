...
use image::{ColorType, DynamicImage, GenericImageView, ImageEncoder, RgbaImage};
...
const AI_IMAGE_MAX_DIMENSION: u32 = 1568;
const AI_IMAGE_JPEG_QUALITY: u8 = 85;

fn encode_image_for_ai(image: RgbaImage) -> Result<String, String> {
    let (width, height) = image.dimensions();
    let max_dim = width.max(height);

    let dynamic = DynamicImage::ImageRgba8(image);
    let resized = if max_dim > AI_IMAGE_MAX_DIMENSION {
        let scale = AI_IMAGE_MAX_DIMENSION as f32 / max_dim as f32;
        let new_w = ((width as f32) * scale).round().max(1.0) as u32;
        let new_h = ((height as f32) * scale).round().max(1.0) as u32;
        dynamic.resize(new_w, new_h, FilterType::Triangle)
    } else {
        dynamic
    };
    ...
    JpegEncoder::new_with_quality(&mut buffer, AI_IMAGE_JPEG_QUALITY)
        .write_image(
            rgb.as_raw(),
            rgb.width(),
            rgb.height(),
...
pub async fn capture_selected_area(
    ...
    let base64_str = encode_image_for_ai(cropped)?;
