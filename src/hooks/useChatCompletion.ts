  ...
  const submit = useCallback(
    async (speechText?: string) => {
      ...
      try {
        ...
        // Cache auto-scroll preference for the duration of this stream
        autoScrollRef.current = getResponseSettings().autoScroll;
        ...
        const baseMessages = updatedMessages.messages;
        let pendingFlush = 0;
        ...
        const flushStream = () => {
          ...
          // Use 'auto' during streaming — smooth scroll triggers expensive
          // layout work on every flushed frame.
          scrollToBottom("auto");
        };
        ...
        try {
          ...
          for await (const chunk of fetchAIResponse({
            ...
            // Coalesce per-chunk renders into one paint frame
            if (!pendingFlush) {
              pendingFlush = requestAnimationFrame(flushStream);
            }
          }
          ...
          // Final flush so trailing chunks land before save
          flushStream();
        } catch (e: any) {
          if (pendingFlush) {
            cancelAnimationFrame(pendingFlush);
        ...
        if (fullResponse) {
          ...
          try {
            // Update in-memory state with the persisted shape, then save in
            // the background — no need to round-trip through the DB.
        ...
        if (!hasPermission) {
          ...
          if (!hasPermissionNow) {
            setState((prev) => ({
              ...prev,
              error:
                "Screen Recording permission required. Please enable it by going to System Settings > Privacy & Security > Screen & System Audio Recording. If you don't see AIIS in the list, click the '+' button to add it. If it's already listed, make sure it's enabled. Then restart the app.",
            }));
